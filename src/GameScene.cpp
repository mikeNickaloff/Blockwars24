...
AbstractGameElement *GameScene::addElement(AbstractGameElement *element, AbstractGameElement *parent)
{
    ...
    const bool wasKnown = isKnownElement(element);

    if (!adoptElement(element, targetParent)) {
        return nullptr;
    }

    if (!wasKnown || !isKnownElement(element)) {
...
AbstractGameElement *GameScene::findElement(const QString &elementId) const
{
    ...
    const auto it = m_elementIndex.constFind(elementId);
    if (it == m_elementIndex.cend()) {
        return nullptr;
    }
    return it->data();
}
...
QPair<AbstractGameElement *, GameSignal> GameScene::dequeueEvent()
{
    ...
    while (!m_eventQueue.isEmpty()) {
        ...
        AbstractGameElement *element = queued.first.data();
        if (!element || !containsElement(element) || !queued.second.isValid() || element->isBlocked()) {
            continue;
        }
        if (m_eventQueue.isEmpty()) {
            emit queuedEventsChanged();
        }
        return {element, queued.second};
    }
...
{
    return element && (element == this || isKnownElement(element));
}
...
QVariantMap GameScene::serializeQueuedEvent(const QPair<AbstractGameElement *, GameSignal> &event) const
{
    ...
    if (!event.first || !containsElement(event.first) || !event.second.isValid()) {
        return serialized;
    }
...
void GameScene::registerElement(AbstractGameElement *element)
{
    if (!element) {
        return;
    }

    if (m_connections.contains(element)) {
        return;
    }

    const QString uniqueId = ensureUniqueId(element);
    m_elementIndex.insert(uniqueId, element);
    m_reverseIndex.insert(element, uniqueId);

    ElementConnections connections;
    connections.idChanged = connect(element, &AbstractGameElement::elementIdChanged, this, [this, element]() {
        handleElementIdChanged(element);
    });
    connections.childAdded = connect(element, &AbstractGameElement::childAdded, this, [this, element](AbstractGameElement *child) {
        handleChildAdded(element, child);
    });
    connections.childRemoved = connect(element, &AbstractGameElement::childRemoved, this, [this, element](AbstractGameElement *child) {
        handleChildRemoved(element, child);
    });
    connections.destroyed = connect(element, &QObject::destroyed, this, [this](QObject *object) {
        if (auto *destroyedElement = qobject_cast<AbstractGameElement *>(object)) {
            handleElementDestroyed(destroyedElement);
        }
    });
    m_connections.insert(element, connections);
...
void GameScene::unregisterElement(AbstractGameElement *element)
{
    if (!element || element == this) {
        return;
    }
    ...
    if (auto reverseIt = m_reverseIndex.find(element); reverseIt != m_reverseIndex.end()) {
        const QString id = reverseIt.value();
        m_reverseIndex.erase(reverseIt);
        auto indexIt = m_elementIndex.find(id);
        if (indexIt != m_elementIndex.end() && indexIt.value().data() == element) {
            m_elementIndex.erase(indexIt);
        }
    } else {
        const QString id = element->elementId();
        auto indexIt = m_elementIndex.find(id);
        if (indexIt != m_elementIndex.end() && indexIt.value().data() == element) {
            m_elementIndex.erase(indexIt);
        }
    ...
    if (auto connectionsIt = m_connections.find(element); connectionsIt != m_connections.end()) {
        QObject::disconnect(connectionsIt->idChanged);
        QObject::disconnect(connectionsIt->childAdded);
        QObject::disconnect(connectionsIt->childRemoved);
        QObject::disconnect(connectionsIt->destroyed);
        m_connections.erase(connectionsIt);
    }
}

void GameScene::handleChildAdded(AbstractGameElement *parent, AbstractGameElement *child)
{
    Q_UNUSED(parent);
    registerElement(child);
}

void GameScene::handleChildRemoved(AbstractGameElement *parent, AbstractGameElement *child)
{
    Q_UNUSED(parent);
    if (!child) {
        return;
...
void GameScene::handleElementDestroyed(AbstractGameElement *element)
{
    unregisterElement(element);
}

void GameScene::handleElementIdChanged(AbstractGameElement *element)
{
    if (!element) {
        return;
    }
    ...
    if (auto reverseIt = m_reverseIndex.find(element); reverseIt != m_reverseIndex.end()) {
        const QString previousId = reverseIt.value();
        auto indexIt = m_elementIndex.find(previousId);
        if (indexIt != m_elementIndex.end() && indexIt.value().data() == element) {
            m_elementIndex.erase(indexIt);
        }
    }
...
QString GameScene::ensureUniqueId(AbstractGameElement *element)
{
    if (!element) {
        return {};
    }

    QString candidate = element->elementId();
    if (!candidate.isEmpty()) {
        const auto existingIt = m_elementIndex.constFind(candidate);
        if (existingIt == m_elementIndex.cend() || existingIt.value().data() == element) {
            return candidate;
        }
        candidate.clear();
    }
    ...
    } while (m_elementIndex.contains(generated));

    if (element->elementId() != generated) {
        element->setElementId(generated);
    }

    return generated;
}

bool GameScene::isKnownElement(const AbstractGameElement *element) const
{
    if (!element) {
        return false;
    }
    return m_connections.contains(const_cast<AbstractGameElement *>(element));
}
